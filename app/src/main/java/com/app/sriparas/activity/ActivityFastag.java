...
import com.app.sriparas.adapter.FastagAdapter;
...
public class ActivityFastag extends AppCompatActivity implements View.OnClickListener,
        FastagAdapter.FastagAdapterListner, updateBalance {

    private static final int REQUEST_LOCATION = 123;
    String remitterId,remitterMobile;
    Boolean fastagStatus;
    RecyclerView recyclerViewBene;
    RelativeLayout rlNoData,rlList;
    TextView txtBalance;
    ProgressBar progressBarBalance;
    ScrollView scrollView;
    FloatingActionButton floatAddNewBeneficiary;
    ImageView imgWallet,imgBack;
    Handler handler;
    UserData userData;
   ...
   // private ArrayList<String> ftg_ifsc;
    ...
    private void initView() {
        ...
        remitterMobile=getIntent().getStringExtra(Constant.REMITTER_MOBILE);
    ...
    public void onClick(View v) {
        ...
        if (v==btnAddBeneficiary){
            ...
            if (Configuration.hasNetworkConnection(ActivityFastag.this)) {
                if (validateBene(account, bank)) {
                    if (!TextUtils.isEmpty(remitterMobile)) {
                        handler.post(() -> addBeneficiary(remitterMobile, account, bkCode, userData.getId(), userData.getTxntoken()));
                    }else {
        ...
        if (v==btnAddNew||v==floatAddNewBeneficiary){
            showGroup();
        }
        if (v == btnKyc) {
            ...
            if (Configuration.hasNetworkConnection(ActivityFastag.this)) {
                if (isValidate(address, city, district, state, pincode)) {
                    if (!remitterMobile.isEmpty()) {
                        handler.post(() -> doKyc(remitterMobile, userData.getFullName(), address, city, district, state, pincode,
                                userData.getId(), userData.getTxntoken()));
                    } else {
                        SweetToast.error(ActivityFastag.this,"Enter mobile number");
                    }
                }
    ...
    private void closeGroup() {
        ...
        getBeneficiary(remitterMobile, userData.getId(), userData.getTxntoken());
        ...
        StringRequest strReq = new StringRequest(Request.Method.POST,
            ...
            try {
                ...
                if  (status.equals("0")) {
                    ...
                    checkUserFastag(mobile,userId,token,userData.getTxntoken());
                } else {
            ...
            if (manager != null) {
                if (!manager.isProviderEnabled(LocationManager.GPS_PROVIDER) && hasGPSDevice(ActivityFastag.this)) {
                    ...
                    if (validateRecharge(lat,lon)){
                        doRechargeFastag(pos, remitterMobile, remitterId, beneficiary_id, beneAccount, amount,
                                lat, lon, userData.getId(), userData.getTxntoken(), dialg);
